...
static t_token_type	handle_pipe(char **input)
{
    (*input)++;
    return (TOKEN_PIPE);
}

static t_token_type	handle_less(char **input)
{
    if (*(*input + 1) == '<')
    {
        *input += 2;
...
static t_token_type	handle_greater(char **input)
{
    if (*(*input + 1) == '>')
    {
        *input += 2;
...
t_token_type	get_operator_type(char **input)
{
    ...
    if (**input == '<')
        return (handle_less(input));
    if (**input == '>')
        return (handle_greater(input));
    return (TOKEN_WORD);
}
